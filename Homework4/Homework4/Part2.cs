...
namespace Homework4
{
    static class Part2
    {
        ...
        {
            list.OfType<Film>()
                .SelectMany(f => f.Actors.Where(a => a.Birthdate.Month == 8)).ToList()
                .ForEach(i => Console.WriteLine(i.Name));
        }
        ...
        public static void Ten(List<object> list)
        {
            list.OfType<Article>()
                ...
                .ToList()
                .ForEach(g => g.ToList()
                    .ForEach(a => Console.WriteLine(g.Key + ": " + a.Name[..20])));
            Console.WriteLine();
        }
    }
}